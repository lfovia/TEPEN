...
class DepthPolicy(nn.Module):
    ...
    def __init__(self,max_token_length=64,device='cuda:1'):
        ...
        self.dt_policy = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=768, nhead=4, dropout=0.25, batch_first=True, norm_first=True, device=device),
            num_layers=4
        )
        ...
        self.action_head = nn.Sequential(nn.Linear(768, 256), nn.ReLU(), nn.Dropout(0.1), nn.Linear(256, 6)).to(device)
...
def random_pixel_goal(habitat_config,habitat_env, difficulty='medium'):
    ...
    if condition_index.shape[0] == 0:
        ...
        return True,np.array(depth),target_mask,target_point,geodesic_distance,[target_x,target_x]

def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--prefix",type=str,choices=['hm3d','mp3d'],default='hm3d')
...
model=DepthPolicy().to(device)

print("Total parameters : ",sum(p.numel() for p in model.parameters()))
print("Trainable p/m : ",sum(p.numel() for p in model.parameters() if p.requires_grad))

print(POLICY_CHECKPOINT)
if os.path.exists(POLICY_CHECKPOINT)==True:
    model.load_state_dict(torch.load(POLICY_CHECKPOINT, weights_only=True))
...
for i in tqdm(range(args.episodes)):
    timesteps = 0
    obs = env.reset()
    goal_flag,goal_image,goal_mask,goal_point,goal_dist,goal_coord = random_pixel_goal(habitat_config,env,args.difficulty)
    ...
    if goal_flag == False or oracle_agent.get_next_action(goal_point) == 0:
        ...
        continue
    max_token_length=64
    history_image = np.zeros((max_token_length,224,224,1))
    ...
    while True:
        ...
        history_image[timesteps] = current_obs[...]
        ...
        action_pred,_,_ = model(goal_mask_tensor, goal_image_tensor, input_image_tensor)
        predicted_actions = torch.argmax(action_pred, dim=-1)
        
        obs=env.step({"action": valid_actions[predicted_actions[0][timesteps].detach().cpu()]})
